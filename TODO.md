...
- [ ] Audit logging of all auth decisions (success and failure)
...
- [ ] Provider startup timeout enforcement (currently configured but not enforced)
...
- [ ] Structured audit log with `project_id`, `session_id`, `caller_cn`, `caller_sub`
...
- [ ] `make dev-setup` one-command dev environment with all certs

## Testing

- [ ] Integration tests: full gRPC round-trip (start → input → events → stop)
